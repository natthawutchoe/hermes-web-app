...
function normalizeState(nextState) {
  return {
    ...
    selectedDateIso: nextState?.selectedDateIso || daysFromNow(0),
...
function describeSelectedDate(iso) {
...
function renderWeek() {
  ...
  if (!state.selectedDateIso || state.selectedDateIso < weekStartIso || state.selectedDateIso > weekEndIso) {
    state.selectedDateIso = todayIso >= weekStartIso && todayIso <= weekEndIso ? todayIso : weekStartIso;
  }
  $("#weekStrip").innerHTML = Array.from({ length: 7 }, (_, index) => {
    ...
    return `
      <button class="day-pill ${iso === todayIso ? "is-today" : ""} ${iso === state.selectedDateIso ? "is-selected" : ""}" data-date="${iso}" type="button" aria-pressed="${iso === state.selectedDateIso}">
...
function renderSelectedDay() {
  const selectedIso = state.selectedDateIso || daysFromNow(0);
  const date = dateFromIso(selectedIso);
  const selectedTasks = tasksForDate(selectedIso);
  const selectedClasses = classesForDay(date.getDay());
  const taskHtml = selectedTasks.length
    ? selectedTasks.map((task) => renderTask(task, true)).join("")
    : `<div class="empty-state">No tasks due on this day.</div>`;
  const classHtml = selectedClasses.length
    ? selectedClasses.map((item) => renderClassCard(item)).join("")
    : `<div class="empty-state">No classes on this day.</div>`;
  $("#nextDeadline").innerHTML = `
    <div class="selected-day-head">
      <strong>${escapeHtml(describeSelectedDate(selectedIso))}</strong>
      <span>${selectedTasks.length} task${selectedTasks.length === 1 ? "" : "s"} · ${selectedClasses.length} class${selectedClasses.length === 1 ? "" : "es"}</span>
    </div>
    <div class="selected-day-list">${taskHtml}${classHtml}</div>
  `;
}
